...
<asp:Content ID="ChatContent" ContentPlaceHolderID="GameContent" runat="server">
    ...
    <div class="panel panel-default" id="SelectRecipientWindow" runat="server">
        <div class="panel-heading">
            <h3 class="panel-title">HackNet Secured Chat</h3>
        </div>
        ...
        <div class="row" style="padding:20px">
            <br />
            <p>
                Our secured realtime chat is highly protected, with multiple layers of 
                encryption standards such as AES-256, RSA and PBKDF2.
            </p>
            <p>
                It's so secure, it's unbreakable without your password.
            </p>
        </div>
                    ...
                    <asp:Repeater ID="ChatRepeater" runat="server" OnLoad="ChatRepeater_Load">
                        <ItemTemplate>
                            <li class="<%# ThisOrOther( (int) Eval("SenderId") ) %>">
                                <div class="msg">
                                    <div class="user">
                                        <asp:Label ID="LblMsgSender" Text='<%# GetUsername( (int) Eval("SenderId") ) %>' runat="server" />
                                    </div>
                                    <p>
                                        <asp:Label ID="LblMsgContent" Text='<%# Eval("Content") %>' runat="server" />
                                    </p>
                                    <time>
                                        <asp:Label ID="LblMsgTimestamp" Text='<%# Eval("Timestamp") %>' runat="server" /></time>
                                </div>
                            </li>
                        </ItemTemplate>
                    </asp:Repeater>
